...
namespace WebApplication.Pages.Metrics
{
    public class CPU_MemoryModel : PageModel
    {
        ...
        private IOptions<AppConfig> appSetting { get; set; }
        public CPU_MemoryModel(FetchDataService fetchDataService,IOptions<AppConfig> options) 
        {
        ...
        public async Task OnGet()
        {
            ...
            List<CPU_Usage> cpu_addOn = await _fetchDataService.getData<CPU_Usage>(oldStamp, newStamp);
            List<Mem_Usage> mem_addOn = await _fetchDataService.getData<Mem_Usage>(oldStamp, newStamp);
